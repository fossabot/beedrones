...
class VirtualMachine(ClskObject):
    ...
    def __init__(self, api_client, data=None, oid=None):
        ...
        # set virtual machine type
        if self._data:
            if 'systemvmtype' in self._data:
                self._type = VirtualMachineType.SYSTEM_VM
            elif 'isredundantrouter' in self._data:
                self._type = VirtualMachineType.ROUTER_VM
            else: 
                self._type = VirtualMachineType.GUEST_VM  
    ...
    def info(self, cache=True):
        ...
        try:
            ...
            res = json.loads(response)['listvirtualmachinesresponse']['virtualmachine'][0]
            self._data = res
